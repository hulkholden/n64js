...
export class SystemTestOutput {
  ...
  consume(line) {
    ...
    if (line.includes('Exception storm detected. Aborting.')) this.status = RunStatus.STORM;
    if (line.includes('Done, but no tests were executed')) this.status = RunStatus.EMPTY;
    ...
    if (this.summaryStarted) {
      ...
      if (line.startsWith('Slowest tests:')) this.status = this.tests > 0 ? RunStatus.COMPLETE : RunStatus.EMPTY;
    }
  }
}
